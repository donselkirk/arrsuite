...
grep -q 'COPILOT_AGENT_TOKEN' "$upstream_workflow"
grep -Fq 'copilot-swe-agent[bot]' "$upstream_workflow"
grep -Fq 'custom_agent:"arrsuite-upstream"' "$upstream_workflow"
grep -q 'actions/upload-artifact@v4' "$upstream_workflow"
grep -q 'target: github-copilot' "${project_root}/.github/agents/arrsuite-upstream.agent.md"
