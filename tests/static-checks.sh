...
grep -q 'SUPPORTED_APPS=(sonarr radarr lidarr prowlarr byparr flaresolverr seerr)' "$install_script"
...
grep -q '\[\[ "$app" == "lidarr" || "$app" == "prowlarr" || "$app" == "byparr" || "$app" == "flaresolverr" || "$app" == "seerr" \]\] && default_state="OFF"' "$install_script"
