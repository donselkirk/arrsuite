...
grep -q 'uv pip install psycopg2-binary' "$install_script"
grep -q 'migrate_bazarr_data()' "$install_script"
...
grep -q 'Upgrade through ${bridge_version} first, then explicitly return to the latest release' "$install_script"
grep -q 'ARRSUITE_UPDATE_BASE_URL="%s" arrsuite self-update' "$install_script"
...
grep -q '## Important changes' "$release_workflow"
...
grep -q 'apt-get install -y shellcheck ripgrep' "$upstream_workflow"
grep -q 'contents: write' "$upstream_workflow"
grep -q 'issues: write' "$upstream_workflow"
