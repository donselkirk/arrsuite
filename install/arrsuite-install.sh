...
readonly -a BACKUP_APPS=(sonarr radarr lidarr prowlarr seerr bazarr)
...
backup_apps() {
  ...
  for app in "${apps[@]}"; do
    ...
    case "$app" in
      seerr) create_seerr_backup "${destination%/}/${app}" || failures=$((failures + 1)) ;;
      bazarr) create_bazarr_backup "${destination%/}/${app}" || failures=$((failures + 1)) ;;
...
restore_native_backup() {
  ...
  backup_file="${2:-}"
  supports_backup "$app" || {
    msg_error "Native restore is supported only for Sonarr, Radarr, Lidarr, and Prowlarr"
    return 2
  }
...
create_bazarr_backup() {
  local destination="$1" data_dir="${APP_DATA_ROOT}/bazarr" timestamp archive temp_archive was_active=0
  is_installed bazarr || {
    msg_error "Bazarr is not installed"
    return 1
  }
  [[ -f "$data_dir/db/bazarr.db" && ! -L "$data_dir/db/bazarr.db" ]] || {
    msg_error "Bazarr SQLite database not found: ${data_dir}/db/bazarr.db"
    return 1
  }

  install -d -m 0750 "$destination"
  timestamp="$(date +%Y.%m.%d_%H.%M.%S)"
  archive="${destination%/}/arrsuite_bazarr_backup_${timestamp}.zip"
  temp_archive="$(mktemp "${destination%/}/.bazarr-backup.XXXXXX")"
  systemctl is-active --quiet bazarr && was_active=1

  msg_info "Creating Bazarr Data Backup"
  if ((was_active)); then
    systemctl stop bazarr || { rm -f "$temp_archive"; return 1; }
  fi
  if ! python3 - "$data_dir" "$temp_archive" <<'PYTHON'
import datetime
import json
import os
import sys
    ...
    for root, directories, files in os.walk(source):
        ...
        for name in files:
            ...
            if not os.path.islink(path):
                archive.write(path, os.path.join("data", os.path.relpath(path, source)))
PYTHON
  then
    rm -f "$temp_archive"
    if ((was_active)); then
  ...
  mv "$temp_archive" "$archive"
  if ((was_active)) && ! systemctl start bazarr; then
    msg_error "Backup was saved to ${archive}, but Bazarr failed to restart"
    return 1
  fi
...
try:
    with zipfile.ZipFile(archive_path) as archive:
        if archive.testzip() is not None:
            raise ValueError("CRC validation failed")
        manifest = json.loads(archive.read("arrsuite-backup.json"))
        if manifest.get("format") != "arrsuite-bazarr-backup" or manifest.get("schema") != 1:
            raise ValueError("unsupported backup format")
        items = archive.infolist()
        names = {item.filename for item in items}
        if "data/db/bazarr.db" not in names:
            raise ValueError("SQLite database is missing")
        if len(items) > 10000 or sum(item.file_size for item in items) > 10 * 1024**3:
            raise ValueError("archive exceeds restore safety limits")
        for item in items:
            ...
            if stat.S_ISLNK(item.external_attr >> 16):
                raise ValueError("symbolic links are not allowed")
            if item.filename != "arrsuite-backup.json" and path.parts[0] != "data":
                raise ValueError("unexpected archive content")
        if extract_path:
            archive.extractall(extract_path)
except (KeyError, OSError, ValueError, zipfile.BadZipFile, json.JSONDecodeError):
...
restore_bazarr_backup() {
  local backup_file="$1" data_dir="${APP_DATA_ROOT}/bazarr" stage_dir rollback_dir
  [[ -f "$backup_file" && -r "$backup_file" && "${backup_file,,}" == *.zip ]] || {
    msg_error "Provide a readable Bazarr backup ZIP"
    return 2
  }
  validate_bazarr_backup_zip "$backup_file" || {
    msg_error "The selected file is not a valid ArrSuite Bazarr backup ZIP"
    return 2
  }
  ...
  rollback_dir="${data_dir}.arrsuite-rollback.$(date +%s).$$"

  msg_info "Restoring Bazarr from ${backup_file}"
  systemctl stop bazarr || { rm -rf "$stage_dir"; return 1; }
  mv "$data_dir" "$rollback_dir" || {
    systemctl start bazarr || true
    rm -rf "$stage_dir"
    return 1
  }
  if ! mv "$stage_dir/data" "$data_dir" || ! systemctl start bazarr || ! systemctl is-active --quiet bazarr; then
    systemctl stop bazarr || true
    rm -rf "$data_dir"
    if ! mv "$rollback_dir" "$data_dir"; then
      msg_error "Bazarr restore and automatic rollback both failed; recovery data remains at ${rollback_dir}"
      rm -rf "$stage_dir"
      return 1
    fi
    systemctl start bazarr || true
    rm -rf "$stage_dir"
    msg_error "Bazarr restore failed; the previous data was restored"
    return 1
  fi

  rm -rf "$rollback_dir" "$stage_dir"
  msg_ok "Restored Bazarr from ${backup_file}"
}
...
restore_backup() {
  ...
  case "$app" in
    seerr)
      ...
      ;;
    bazarr)
      require_installed_app bazarr || return
      restore_bazarr_backup "${2:-}"
      ;;
...
# Generated from apps/radarr.sh. Do not edit this block directly.
...
# Generated from apps/lidarr.sh. Do not edit this block directly.
...
# Generated from apps/prowlarr.sh. Do not edit this block directly.
...
# Generated from apps/byparr.sh. Do not edit this block directly.
...
# Generated from apps/flaresolverr.sh. Do not edit this block directly.
...
# Generated from apps/seerr.sh. Do not edit this block directly.
write_seerr_service() {
...
# Generated from apps/bazarr.sh. Do not edit this block directly.
...
remove_or_reset_apps() {
  ...
  if ((${#apps[@]} == 0)); then
    if [[ "$action" == "remove" ]]; then
      msg_error "Usage: arrsuite remove app [app ...] [--purge] [--yes]"
    else
      msg_error "Usage: arrsuite reset app [app ...] [--yes]"
    fi
    return 2
  fi
...
version_is_older() {
  ...
  [[ "$current" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || return 2
  [[ "$required" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || return 2
  [[ "$current" != "$required" ]] || return 1
  first="$(printf '%s\n%s\n' "$current" "$required" | sort -V | sed -n '1p')"
  [[ "$first" == "$current" ]]
}
...
self_update() {
  ...
  if ! curl -fsSL --retry 3 --retry-all-errors "${asset_url}/SHA256SUMS" -o "${temp_dir}/SHA256SUMS"; then
    rm -rf "$temp_dir"
    msg_error "Failed to download the ArrSuite release checksums"
    return 1
  fi

  if grep -q ' COMPATIBILITY$' "${temp_dir}/SHA256SUMS"; then
    if ! curl -fsSL --retry 3 --retry-all-errors "${asset_url}/COMPATIBILITY" -o "${temp_dir}/COMPATIBILITY" \
      || ! (cd "$temp_dir" && sha256sum -c --ignore-missing SHA256SUMS >/dev/null); then
      rm -rf "$temp_dir"
      msg_error "Failed to download or verify ArrSuite upgrade compatibility metadata"
      return 1
    fi
  ...
  if version_is_older "$release_version" "$current_version"; then
    rm -rf "$temp_dir"
    msg_error "Refusing to downgrade ArrSuite from ${current_version} to ${release_version}"
    return 1
  fi
  ...
  if ! curl -fsSL --retry 3 --retry-all-errors "${asset_url}/arrsuite-manager" -o "${temp_dir}/arrsuite" \
    ...
    || ! curl -fsSL --retry 3 --retry-all-errors "${asset_url}/fix-console-autologin.sh" -o "${temp_dir}/fix-console-autologin.sh"; then
    rm -rf "$temp_dir"
...
Usage:
  ...
  arrsuite backup [app ...] [--output directory]
                               Back up Sonarr, Radarr, Lidarr, Prowlarr, Seerr, and/or Bazarr
  arrsuite restore app backup.zip
                               Restore a Sonarr, Radarr, Lidarr, Prowlarr, Seerr, or Bazarr backup
