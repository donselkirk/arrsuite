...
readonly -a BACKUP_APPS=(sonarr radarr seerr)
...
configure_app_api() {
  ...
  APP_API_URL="${APP_WEB_URL}/api/v3"
}
...
try:
    with zipfile.ZipFile(sys.argv[1]) as archive:
        ...
        if expected not in names and "nzbdrone.db" not in names:
...
PYTHON
  then
    rm -f "$temp_archive"
    ((was_active)) && systemctl start seerr || true
...
restore_native_backup() {
  ...
  backup_file="${2:-}"
  supports_backup "$app" || {
    msg_error "Native restore is supported only for Sonarr and Radarr"
    return 2
  }
  is_installed "$app" || {
    msg_error "${APP_LABEL[$app]} is not installed"
...
Usage:
  ...
  arrsuite backup [app ...] [--output directory]
                               Back up Sonarr, Radarr, and/or Seerr
  arrsuite restore app backup.zip
                               Restore a Sonarr, Radarr, or Seerr backup
...
main() {
  case "${1:-help}" in
    ...
    restore)
      ...
      shift
      (($# == 2)) || {
        msg_error "Usage: arrsuite restore sonarr|radarr|seerr backup.zip"
        exit 2
      }
      restore_backup "$1" "$2"
      ;;
