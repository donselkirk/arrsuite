...
readonly -a BACKUP_APPS=(sonarr radarr)
...
backup_apps() {
  ...
  for app in "${apps[@]}"; do
    if ! supports_native_backup "$app"; then
      msg_error "Native backup is not supported for: ${app}"
      failures=$((failures + 1))
      continue
    fi
...
Usage:
  ...
  arrsuite backup [app ...] [--output directory]
                               Create native backups for Sonarr and/or Radarr
  arrsuite restore app backup.zip
                               Restore a native Sonarr or Radarr backup
...
main() {
  case "${1:-help}" in
    ...
    restore)
      ...
      shift
      (($# == 2)) || {
        msg_error "Usage: arrsuite restore sonarr|radarr backup.zip"
        exit 2
      }
      restore_native_backup "$1" "$2"
      ;;
    list)
      show_list
      ;;
