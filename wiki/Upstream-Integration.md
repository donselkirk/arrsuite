...
repatched to use ArrSuite's release-pinned helper base, validated, and proposed
on the stable `automation/upstream-review` branch.
...
`upstream-review/pending.md` without advancing their locks. Pull-request
validation remains blocked until each application module is adapted and tested
...
1. Review the automated PR and its focused report.
2. For pending application items, update the relevant `apps/<app>.sh` and
   templates, then update that application's locked source blob.
